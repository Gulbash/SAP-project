...
```{r}
hist(Duration, col = 'lightgreen', ylim = c(0, 300), main = 'Histogram varijable "Duration"')
...
hist(defaulted$Duration, breaks = 10, xlim = c(0, 75), ylim = c(0, 250), col = rgb(1, 0, 0, 0.5),
     xlab = 'Duration', ylab = 'Frequency', main = 'Distribucija varijable "Duration" prema varijabli "Default"' )
...
```{r}
hist(CreditAmount, col = 'lightgreen', ylim = c(0, 500), main = 'Histogram varijable "Duration"')

boxplot(CreditAmount, col = 'lightgreen', main = 'Boxplot varijable "Duration"')
...
plot(CreditAmount, Duration,
     ...
     cex = 1, 
     col = Default,
     xlab = 'CreditAmount', ylab = 'Duration',
     main = 'Odnos varijabli "CreditAmount" i "Duration"'
)
legend("topright", legend = c('nije "Default"', '"Default"'), col = Default, pch = 16)
...
Vidimo da se većina malih kredita s kratkim trajanjem vraća. S druge strane, dugotrajni krediti s većim iznosima češće budu neplaćeni. Veličina mjesečne rate ne čini se da ima značajan utjecaj na našu ciljanu varijablu. Kako bismo zaključili naš popis koreliranih varijabli, provjerimo posljednje dvije, "CreditHistory" i "EmploymentSince":
...
print("Postotak mušakraca koji nisu redovito ispunjavali obaveze oprema banci:")
print(as.numeric(1-table(male$Default)/count(male)))
print("Postotak žena koje nisu redovito ispunjavali obaveze oprema banci:")
...
for (item in unique(data$Property)) {
  hist(data$CreditAmount[data$Property==item], main = paste("", item), xlab = "CreditAmount")
}
...
Bartlettovim testom testiramo homogenost varijanci kod razlicitih populacija. Postavljamo i poteze:
$$ \begin{aligned}
...
Vidimo da su pretpostavke o normalnosti i pretpostavka o homogenosti valjane ako pogledamo pvrijednosti testova. 
